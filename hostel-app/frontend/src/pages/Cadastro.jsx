...
import { Mail, Lock, AlertCircle, Loader2, User, Hotel } from 'lucide-react'
...
export default function Cadastro() {
  ...
  const submit = async (e) => {
    ...
    setLoading(true)
    try {
      // Chama a função de cadastro na sua API
      ...
      setErro(err.response?.data?.erro || err.message || 'Erro ao realizar o cadastro. Tente novamente.')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="text-center relative z-10 w-full max-w-md px-6 animate-fade-in">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-teal-500/15 border border-teal-500/30 mb-4 shadow-lg shadow-teal-500/10">
            <Hotel className="w-8 h-8 text-teal-400" />
          </div>
          <h1 className="text-3xl font-display font-bold text-white tracking-tight">HostelApp</h1>
          <p className="text-slate-400 text-sm mt-1 font-body">Sistema de Gestão de Hotel</p>
        </div>
        ...
        <p className="text-center text-xs text-slate-600 mt-6 font-body">
          © {new Date().getFullYear()} HostelApp — Todos os direitos reservados
        </p>
      </div>
    </div>
  )
