...
function HospedeModal({ form, setForm, onSave, onClose, editId, loading }) {
      ...
      <div className="glass w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-slide-up max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 shrink-0">
          <h3 className="font-display font-semibold text-white flex items-center gap-2">
            ...
            {editId ? 'Editar Hóspede' : 'Novo Hóspede'}
          </h3>
        ...
        <form onSubmit={submit} className="overflow-auto flex-1 px-6 py-5">
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input name="nome" value={form.nome} onChange={handle} placeholder="Nome do hóspede" className="input-base" />
            </div>
            <div>
            ...
            <div className="col-span-2">
              <label className="label">Observações Médicas / Restrições</label>
              <input name="observacoes" value={form.observacoes} onChange={handle} placeholder="Ex: Alergia a amendoim, precisa de berço..." className="input-base" />
            </div>
          </div>
...
export default function Hospedes() {
  ...
  const carregar = useCallback(async () => {
    try { setHospedes(await api.getHospedes()) } catch { toast.error('Erro ao carregar hóspedes.') }
  ...
  const openEdit = (h) => {
    setForm({ nome:h.nome, email:h.email||'', telefone:h.telefone||'', cpf:h.documento||h.cpf||'', endereco:h.endereco||'', cidade:h.cidade||'', uf:h.uf||'', observacoes:h.observacoes||'' })
    setEditId(h.id); setShowModal(true)
  }

  const salvar = async () => {
    setSaving(true)
    try {
      if (editId) await api.atualizarHospede(editId, form)
      else        await api.criarHospede(form)
      setShowModal(false)
      toast.success(editId ? 'Hóspede atualizado!' : 'Hóspede cadastrado com sucesso!')
      await carregar()
    } catch (err) {
  ...
  const confirmarDesativar = async () => {
    try {
      await api.deletarHospede(confirmId)
      toast.success('Hóspede desativado.')
      await carregar()
    } catch { toast.error('Erro ao desativar hóspede.') }
    finally { setConfirmId(null) }
  }

  const lista = hospedes.filter(h =>
    h.nome.toLowerCase().includes(busca.toLowerCase()) ||
    (h.email||'').toLowerCase().includes(busca.toLowerCase())
  )

  return (
    <Layout title="Hóspedes" subtitle="Cadastro e histórico de hóspedes">
      ...
      <p className="text-xs text-slate-600 mb-4 font-medium">{lista.length} hóspede{lista.length !== 1 ? 's' : ''}</p>
      ...
      {loading ? (
        ...
        <div className="glass rounded-2xl p-16 text-center">
          ...
          <p className="text-slate-400 font-medium">Nenhum hóspede encontrado.</p>
        </div>
      ) : (
            ...
            <div key={h.id}
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="w-10 h-10 rounded-xl bg-teal-500/15 border border-teal-500/20 flex items-center justify-center shrink-0">
                  <span className="text-teal-400 font-display font-bold text-sm">{h.nome[0].toUpperCase()}</span>
                </div>
                <div className="flex gap-1">
                  <button onClick={() => openEdit(h)} className="p-1.5 text-slate-500 hover:text-teal-400 hover:bg-teal-500/10 rounded-lg transition-all" title="Editar">
                    <Pencil className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={() => setConfirmId(h.id)} className="p-1.5 text-slate-500 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-all" title="Desativar">
                    <Trash2 className="w-3.5 h-3.5" />
                  </button>
                </div>
              </div>
              <h3 className="font-display font-semibold text-white text-sm leading-tight">{h.nome}</h3>
              {(h.documento || h.cpf) && (
                <p className="text-xs text-slate-500 mt-0.5 font-mono">{h.documento || h.cpf}</p>
              )}
              <div className="mt-3 space-y-1.5">
                {h.email    && <p className="text-xs text-slate-400 flex items-center gap-1.5 truncate"><Mail   className="w-3 h-3 text-slate-600 shrink-0" />{h.email}</p>}
                {h.telefone && <p className="text-xs text-slate-400 flex items-center gap-1.5"><Phone  className="w-3 h-3 text-slate-600 shrink-0" />{h.telefone}</p>}
                {h.cidade   && <p className="text-xs text-slate-400 flex items-center gap-1.5"><MapPin className="w-3 h-3 text-slate-600 shrink-0" />{h.cidade}{h.uf?`, ${h.uf}`:''}</p>}
              </div>
            </div>
          ))}
        </div>
      ...
      {showModal && (
        <HospedeModal form={form} setForm={setForm} onSave={salvar} onClose={() => setShowModal(false)} editId={editId} loading={saving} />
      )}
      {confirmId && (
        <ConfirmDialog
          title="Desativar Hóspede"
          message="O hóspede será desativado. Suas reservas anteriores continuarão registradas no histórico."
